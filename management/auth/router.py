...
from management.auth.auth import get_password_hash, authenticate_user, create_token, \
    get_user_id_from_token
from management.auth.conf import fastmail
from management.auth.schemas import SToken, SUserId, SResetPass, SEmailSchema
...
async def signup_user(user_data: SUser):
    ...
    await UserData.add(email=user_data.email,
...
async def login_user(response: Response, user_data: OAuth2PasswordRequestForm = Depends()):
...
async def reset_password_data(user_data: SResetPass):
    ...
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
    await UserData.user_update(user.id, password=get_password_hash(user_data.new_password))
    return JSONResponse(status_code=200, content={"message": "password has been updated"})
